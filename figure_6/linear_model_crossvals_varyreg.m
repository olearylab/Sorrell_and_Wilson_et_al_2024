function [all_r2_mat,trial_avs_all,trial_av_r2,binned_r2,trial_av_cd_r2,trial_av_rmse] = linear_model_crossvals_varyreg(virmen_data,zdata,tbt_details,model_params,num_subs,extra_norm,lambda_mat,zscore_x,nbins)
% 23/11/2023
...
all_r2_mat = nan.*ones(num_subs,2,4,zdim);
binned_r2 = nan.*ones(num_subs,2,4,zdim);
...
trial_avs = NaN(num_subs,2,2,nbins,zdim);
trial_avs_in = NaN(num_subs,2,2,nbins,zdim);
trial_avs_cross = NaN(num_subs,2,2,nbins,zdim,nModels);
trial_av_r2 = NaN(num_subs,2,4,zdim);
trial_av_cd_r2 = NaN(num_subs,2,4,zdim);
trial_av_rmse = NaN(num_subs,2,4,zdim);
